    ...
    <section className="py-20 bg-gradient-to-br from-blue-50 via-white to-teal-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="bg-white rounded-3xl shadow-2xl overflow-hidden">
            ...
            <div className="bg-gray-50 px-12 py-6 flex items-center justify-between">
              ...
              >
                ...
                <span>{t('testimonials.navigation.previous')}</span>
              </button>
              ...
              <button
                ...
                className="flex items-center space-x-2 text-gray-600 hover:text-teal-600 transition-colors"
              >
                <span>{t('testimonials.navigation.next')}</span>
        ...
        <div className="text-center">
          <p className="text-gray-600 mb-8">{t('testimonials.trustedBy')}</p>
        ...
        <div className="mt-16 bg-white rounded-2xl p-8 shadow-lg">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8 text-center">
            <div>
              <div className="text-3xl font-bold text-blue-600 mb-2">{t('testimonials.stats.customers').split(' ')[0]}</div>
              <div className="text-gray-600">{t('testimonials.stats.customers').split(' ').slice(1).join(' ')}</div>
            </div>
            <div>
              <div className="text-3xl font-bold text-teal-600 mb-2">{t('testimonials.stats.uptime').split(' ')[0]}</div>
              <div className="text-gray-600">{t('testimonials.stats.uptime').split(' ').slice(1).join(' ')}</div>
            </div>
            <div>
              <div className="text-3xl font-bold text-purple-600 mb-2">{t('testimonials.stats.messages').split(' ')[0]}</div>
              <div className="text-gray-600">{t('testimonials.stats.messages').split(' ').slice(1).join(' ')}</div>
            </div>
            <div>
              <div className="text-3xl font-bold text-green-600 mb-2">{t('testimonials.stats.rating').split(' ')[0]}</div>
              <div className="text-gray-600">{t('testimonials.stats.rating').split(' ').slice(1).join(' ')}</div>
            </div>
          </div>
        </div>
      </div>
